...
#include "scene_graph/camera_cfg.hpp"
#include "descriptor_layout_config.hpp"
#include "GPU/context.hpp"
ResourcePool::ResourcePool(const ResourceManagerCreateInfo &info) :
  device_(info.device),
  allocator_(*info.allocator),
  maxInflight_(info.maxInflight),
  descriptorUploader_(info.device)
{
  vkGetPhysicalDeviceProperties(gpu::ctx__.physicalDeviceh__, &physicalDeviceProperties);
  ...
  descriptorManager   = std::make_unique<DescriptorManager>(gpu::ctx__.deviceh__);
