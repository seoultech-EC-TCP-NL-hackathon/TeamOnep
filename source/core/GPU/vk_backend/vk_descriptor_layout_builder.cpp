...
#include "boot/boot_resource.hpp"
#include "vk_descriptor_layout_builder.hpp"

namespace gpu
{
  VkDescriptorLayoutBuilder::VkDescriptorLayoutBuilder(VkContext* pCtxt) : pCtxt(pCtxt)
  {
  }

  VkDescriptorLayoutBuilder::~VkDescriptorLayoutBuilder() = default;

  VkDescriptorSetLayout VkDescriptorLayoutBuilder::createDescriptorSetLayout(std::vector<VkLayoutBindingInfo>& infos)
  {
    ...
    for (VkLayoutBindingInfo& info : infos)
    {
      switch (info.usage)
      {
