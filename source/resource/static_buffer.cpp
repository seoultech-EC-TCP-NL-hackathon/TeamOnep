...
StaticBuffer::StaticBuffer(gpu::VkMemoryAllocator &allocator, VkDeviceSize bufferSize, BufferType type) : allocator(allocator),
...
void StaticBuffer::createMainBuffer()
{
  ...
  if (vkBindBufferMemory(device, buffer, allocation.memory__, allocation.offset__) != VK_SUCCESS)
  {
    spdlog::info("falil to bind buffer to memory__");
  }
}
...
{ ///
  ...
  vkBindBufferMemory(device, stagingBuffer, stagingAllocation.memory__, stagingAllocation.offset__);
  void *bufferData;
  vkMapMemory(device, stagingAllocation.memory__, stagingAllocation.offset__, bufferSize, 0, &bufferData);
  memcpy(bufferData, data, (size_t) bufferSize);
  vkUnmapMemory(device, stagingAllocation.memory__);
}
