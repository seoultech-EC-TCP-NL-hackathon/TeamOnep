...
void SwapchainViewManager::setDepthView()
{
  ...
  for (int i = 0; i < depthImage.size(); i++)
  {
    ...
    if (vkBindImageMemory(device, depthImage[i], allocations[i].memory__, allocations[i].offset__) != VK_SUCCESS)
    {
...
gpu::VkAllocation  SwapchainViewManager::memoryBind(uint32_t index)
{
